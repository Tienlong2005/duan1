    ...
    public function createSanPham(){
        if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['them-san-pham'])){
         ...
         if(empty($_POST['danh_muc_id'])){
            $errors['danh_muc_id'] = 'Không được để trống trường này';
         }
         ...
         $hinh_anh = $file['name'];
         if(move_uploaded_file($file['tmp_name'] , './hinh_anh/anhsanpham/' . $hinh_anh)){
            $addSanPham =$this-> addSanPham($_POST['ten_san_pham'],$_POST['gia_san_pham' ],$_POST['gia_khuyen_mai'], $hinh_anh ,$_POST['so_luong'], $_POST['luot_xem'],  $_POST['ngay_nhap'], $_POST['mo_ta'], $_POST['danh_muc_id'], $_POST['trang_thai']);
             if($addSanPham){
                $_SESSION['susses'] = 'thêm sản phẩm thành công';
